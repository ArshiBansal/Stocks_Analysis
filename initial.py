    ...
    try:
        with st.spinner(f"Fetching latest {ticker} data from Yahoo Finance..."):
            ...
            if data.empty:
                ...
                st.warning(f"No data returned by Yahoo Finance for {ticker}. Please verify the ticker symbol.")
                return pd.DataFrame()
            if data.index.tz is not None:
                data.index = data.index.tz_localize(None)
            data.reset_index(inplace=True)
            logger.info("Data loaded via yfinance for %s", ticker)
            return data
    except (ValueError, KeyError, ConnectionError) as e:
        logger.error("yfinance data load failed for %s: %s", ticker, e)
        st.error("Unable to fetch stock data from Yahoo Finance. Please verify the ticker symbol, check your internet connection, or try again later.")
        return pd.DataFrame()
    ...
    if st.button("Train LSTM + GRU", type="primary", use_container_width=True):
        with st.spinner("Training deep models..."):
            ...
            with st.expander("LSTM"):
                try:
                    lstm_model = Sequential([LSTM(100, return_sequences=True, input_shape=(lookback_days,1)), Dropout(0.3), LSTM(100), Dropout(0.3), Dense(1)])
            ...
            with st.expander("GRU"):
                try:
                    gru_model = Sequential([GRU(100, return_sequences=True, input_shape=(lookback_days,1)), Dropout(0.3), GRU(100), Dropout(0.3), Dense(1)])
